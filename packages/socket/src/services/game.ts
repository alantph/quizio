import { Answer, Player, Quizz } from "@quizio/common/types/game";
import { Server, Socket } from "@quizio/common/types/game/socket";
import {
  Status,
  STATUS,
  StatusDataMap,
} from "@quizio/common/types/game/status";
import { usernameValidator } from "@quizio/common/validators/auth";
...
import { createInviteCode, timeToPoint } from "@quizio/socket/utils/game";
import sleep from "@quizio/socket/utils/sleep";
...
class Game {
  ...
  showResults(question: any) {
    ...
    const playersBreakdown = sortedPlayers.map((player) => {
      const playerAnswer = this.round.playersAnswers.find(
        (a) => a.playerId === player.id,
      );
      return {
    ...
    this.roundHistory.push({
      ...
      correctAnswerIndex: question.solution,
      answers: question.answers,
      playerResults: sortedPlayers.map((player) => {
        const pa = this.round.playersAnswers.find(
          (a) => a.playerId === player.id,
        );
        return {
          username: player.username,
          answerId: pa ? pa.answerId : null,
          correct: player.lastCorrect,
          points: player.lastPoints,
        };
      }),
  ...
  selectAnswer(socket: Socket, answerId: number) {
    ...
    this.io.to(this.manager.id).emit("manager:playerAnswered", {
      playerId: player.id,
      username: player.username,
      answeredCount: this.round.playersAnswers.length,
